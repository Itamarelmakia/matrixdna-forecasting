"""
MatrixDNA / ItaStyle — Data Cleaning & Pre-processing
-----------------------------------------------------

This script loads three raw tables (`item_spec.csv`, `order_spec.csv`, `customer_spec.csv`),
inspects them, fixes types/missing values, standardizes Online/store/region logic, enriches
orders from items, engineers simple features, removes duplicates, and writes cleaned outputs.

Why: Clean, consistent inputs are a prerequisite for trustworthy EDA and forecasting.

Main functions (what they do)
-----------------------------
1) load_data(data_dir)
   - Reads the three CSVs from `data_dir` into pandas DataFrames.

2) report_missing(df, name)
   - Prints a sorted summary of missing values (count and %) per column; returns the summary table.

3) convert_types(order_df)
   - Parses `order_date` to datetime.
   - Converts count-like numeric columns (`total_items_net`, `num_items`, `num_products`, `quantity`)
     to nullable integers (`Int64`) to preserve NAs without losing type information.

4) clean_categorical(order_df)
   - Fills common categorical NAs with "Unknown" (e.g., `top_location`, `channel`, `gender`) to avoid
     dropping rows and to keep categories explicit.

5) fill_online_store_region(item_df)
   - Normalizes Online rows: sets `store_id = "Online"` and `region = "Online"`.
   - (Optional hook for region normalization, e.g., Hebrew → English mapping if needed.)

6) fill_missing_category_sku_heuristic(item_df)
   - Three-step category completion for `item_spec`:
     (a) SKU→category (mode), (b) subcategory→category (mode), (c) text heuristic on `subcategory`
     (removes color/size tokens), while logging how NA counts change after each step.

7) enrich_orders_with_online_and_topcat(orders_df, items_clean)
   - Detects Online orders (flag or channel) → sets `store_id = "Online"`, `region = "Online"`.
   - Normalizes list-ish cells (`store_id`, `region`, `channel`) into clean strings.
   - Builds/repairs `top_category` using `items_id_list` joined to `items_clean` categories.
   - De-duplicates comma-separated strings for `top_region`, `top_location`, `top_category`.
   - Rounds `avg_amount_per_item` to 2 decimals if present.

8) engineer_features(order_df)
   - Adds lightweight features: `year_month`, `month`, `quarter` from `order_date`.
   - Creates `is_online` boolean (from `channel`).
   - Computes `avg_amount_per_item = line_amount / total_items_net` with safe division (NaN when invalid).

9) clean_customer(customer_df)
   - For count/amount-like columns (name contains `count|num|orders|spend|purchase`):
     fills missing with 0 (interpreting “no activity in window” as zero, not NA).

10) drop_duplicates(df, key, name)
    - Removes duplicate rows by key(s) and logs how many were dropped.

11) _factorize_key(df, cols, name)
    - Utility: builds a consistent integer surrogate key from a multi-column signature
      (useful later for dimensional modeling).

Pipeline summary (end-to-end)
-----------------------------
- Load → Inspect → Missing-value report
- Orders: convert_types → clean_categorical → engineer_features → deduplicate
- Items: fill_online_store_region → fill_missing_category_sku_heuristic → deduplicate
- Orders ⇐ enrich from Items (Online fields, top_category, string normalization)
- Customers: clean_customer → deduplicate
- Save cleaned tables to Excel (one workbook, multiple sheets) for downstream EDA & modeling

Run
---
>>> python data_cleaning.py

Dependencies: pandas, numpy (no external services).
"""
...
def main():
    ...
    DATA_DIR = os.path.join(base_dir, "matrixdna_cleaned_data.xlsx")


    item_spec, order_spec, customer_spec = load_data(DATA_DIR)
    ...
    output_excel_path = os.path.join(output_dir, 'matrixdna_cleaned_data.xlsx')
