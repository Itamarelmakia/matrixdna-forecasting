...
# ----------------- Config (from cfg) -----------------
# Expect cfg to be passed into run(cfg) or set earlier in this module.
PROCESSED_XLSX = Path(cfg["paths"]["processed_xlsx"])
OUTDIR = Path(cfg["paths"]["forecast_out"]); OUTDIR.mkdir(parents=True, exist_ok=True)

TEST_H  = int(cfg["modeling"]["test_h"])         # Last N months for validation (backtest)
FUTURE_H = int(cfg["modeling"]["future_h"])      # Months to forecast beyond the last actual

# What-if multipliers
WHAT_IF_ORDERS_MULT = float(cfg["modeling"]["what_if_orders_mult"])
WHAT_IF_AOV_MULT    = float(cfg["modeling"]["what_if_aov_mult"])
...
def run_pipeline():
    ...
    prev_year_stats = monthly.loc["2023-08":"2024-01", "total_revenue"].agg(["mean", "median"])
